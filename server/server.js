...
app.get("/api/player/tutorial", async (req, res) => {
  ...
  try {
    ...
    console.error("Tutorial status lookup failed:", error);

    return res.status(error.status || 500).json({
      ok: false,
      error: error.message || "Could not check tutorial status.",
    });
  }
});

/**
 * Mark the guided tutorial complete for this Discord account.
 */
app.post("/api/player/tutorial/complete", async (req, res) => {
  if (!isDatabaseReady()) {
    return res.status(503).json({
      ok: false,
      error: "Database is still connecting.",
  ...
  try {
    const user = await getDiscordUserFromBearer(req);
    const profile = await markTutorialComplete(user.id);
    ...
    console.error("Tutorial completion save failed:", error);

    return res.status(error.status || 500).json({
      ok: false,
      error: error.message || "Could not save tutorial completion.",
    });
  }
});

/**
 * Admin-only tutorial reset for the authenticated admin account.
 */
app.delete("/api/admin/tutorial", async (req, res) => {
  if (!isDatabaseReady()) {
    return res.status(503).json({
      ok: false,
      error: "Database is still connecting.",
  ...
  try {
    ...
    if (!adminId || user.id !== adminId) {
      return res.status(403).json({
        ok: false,
        error: "Only the configured Distortion Grid admin can reset the tutorial.",
      });
    }

    const profile = await resetTutorialComplete(user.id);
    ...
    console.error("Admin tutorial reset failed:", error);

    return res.status(error.status || 500).json({
      ok: false,
      error: error.message || "Could not reset tutorial status.",
    });
  }
});
